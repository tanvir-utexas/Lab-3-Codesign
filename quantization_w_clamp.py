...
def quantizeLayer(x, layer, stat, scale_x, zp_x, num_bits=8, weights_save_name=None):
  ...
  if weights_save_name:
      np.save(f'./weights_npy/fp_w_{weights_save_name}_clamp', W.cpu().numpy())
      qt_w = w.scale*(w.tensor - w.zero_point)
      np.save(f'./weights_npy/quant_w_{weights_save_name}_clamp', qt_w.cpu().numpy())
...
def clamp_weights(model, ratio=0.8):

    for p in model.parameters():
        param = p.data
        min_val, max_val = param.min(), param.max()
        param = param.clamp(ratio*min_val, ratio*max_val)
        p.data = param
...
clamp_weights(q_model, ratio=0.8)
